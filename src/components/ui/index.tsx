...
const LOGO_SRC = 'https://res.cloudinary.com/dvbdvkhs/image/upload/v1783017338/Screenshot_2026-07-03_at_12.05.26_AM_xugy0b.png'

export function Logo({ size = 'md', hide = false }: { size?: 'sm' | 'md' | 'lg' | 'xl', hide?: boolean }) {
  ...
  return (
    <div className={clsx('flex items-center justify-center gap-2.5 font-semibold tracking-tight text-gray-900', sizes[size])}>
      <div className={clsx('relative flex flex-shrink-0 items-center justify-center overflow-hidden', iconSizes[size])}>
        <img src={LOGO_SRC} alt="" className="absolute inset-0 h-full w-full object-cover " />
      </div>
      {!hide && <span className="font-bold">IntelliMeet</span>}
    </div>
  )
}
